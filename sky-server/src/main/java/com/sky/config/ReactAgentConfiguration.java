...
订单查询工具使用说明：
                ...
                - 订单查询工具会返回结构化数据，你需要将返回的数据以JSON格式原样输出，格式：```json {"componentType":"ORDER_LIST",...} ```
                - 不要修改订单查询工具返回的JSON数据，直接输出即可
                ...
                1. 使用中文回答
                2. 数据展示清晰，必要时使用表格或列表
                3. 对于订单查询，工具返回的结构化数据会自动渲染为交互式组件，你只需简要说明查询结果
                4. 对于知识库相关问题，严格基于检索到的信息回答，不要添加个人推断
                5. 保持专业、友好的语气
                6. 今天的日期是 %s
                """, todayDate);
