...
public class AIChatController {
    ...
    @ApiOperation("ai聊天流式输出")
    public SseEmitter chatSteam(@RequestBody AIChatMessageDTO aiChatMessageDTO) {
        log.info("用户输入（流式）：{}", aiChatMessageDTO);

        SseEmitter emitter = new SseEmitter(0L);

        aiChatService.chatSteam(aiChatMessageDTO).subscribe(
                event -> sendEvent(emitter, event),
                error -> {
                    log.error("SSE发送失败", error);
    ...
    private void sendEvent(SseEmitter emitter, AIChatStreamEventVO event) {
        try {
            emitter.send(
                    SseEmitter.event()
                            .name(event.getType())
